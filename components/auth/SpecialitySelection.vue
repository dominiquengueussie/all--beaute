...
const becomeProvider = (formDataProvider) => {
  ...
  try {
    ...
    console.log("Données du prestataire :", prestataireData.value.fullName);
    const selected = specializations.value.find(
      (s) => s.id === selectedSpecialization.value
    );
    const selectedCatNames = selectedCategories.value.join(", ") || "Aucune";
    console.log(
      `Sélection validée :\nSpécialisation: ${selected?.name}\nCatégories: ${selectedCatNames}`
    );
  } catch (error) {
...
<template>
  ...
  >
    <div class="w-full bg-white rounded-xl shadow-lg overflow-hidden">
      ...
      <!-- Content -->
      <div
        class="p-6 mx-auto max-w-2xl max-h-[calc(100vh-150px)] overflow-y-auto"
      >
        <!-- Chargement / Erreur -->
        ...
        <div v-else>
              ...
              >
                <label class="flex items-center space-x-4 cursor-pointer">
                  <div class="relative">
                    ...
                    >
                      <div
                        v-if="selectedSpecialization === spec.id"
          ...
          <transition name="fade">
            <div
              v-if="selectedSpecialization && currentCategories.length"
              class="mb-8 bg-gray-50 rounded-lg p-5 border border-gray-200"
            >
              <h4
                class="text-md font-medium text-gray-700 mb-3 flex items-center"
              >
          ...
          <div class="pt-4 border-t border-gray-200">
            <button
              ...
              :class="{ 'opacity-50 cursor-not-allowed': shouldDisableButton }"
            >
              <span>Continuer</span>
