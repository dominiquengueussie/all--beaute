        ...
        <div class="p-6">
            <div class="flex items-center justify-start text-[16px]">
                ...
                :src="professional.medias[0]?.link || '/placeholder-image.jpg'" />
                 <h4 class="font-semibold text-white">{{ professional.enterprise.name.slice(0, 20) + "..."}} </h4>
            </div>

            <h4 class="font-semibold text-white text-[13px] mt-3">{{ professional.name.slice(0, 25) + "..." }}</h4>
            <div class="flex items-center gap-2 mt-4 text-gray-500 text-sm">
                <span>📍</span>
                {{ professional.enterprise.address.slice(0, 25) + "..." }}
            </div>
