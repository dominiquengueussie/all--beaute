...
const {
    data: apiResponse2,
    execute: execute2,
} = useFetch(() => `${apiEndpoint2}?page=${currentPage.value}&perPage=${perPage.value}`, {
    method: "GET",
    headers: {
        "Content-Type": "application/json",
        accept: "application/json",
...
const { allService, paginationCart, goToPageCart, currentPageCart } = defineProps({
    allService: Array,
...
<template>
    <div class="bg-gray-50 font-sans min-h-screen">
        ...
        <div class="relative bg-gradient-to-r from-purple-900 to-indigo-800 h-64 flex items-center justify-center">
            <div class="absolute inset-0 bg-black opacity-40"></div>
            <div class="relative z-10 text-center px-4">
                <h1 class="text-4xl font-bold text-white mb-2 animate-on-scroll">Détente & Bien-être</h1>
                <p class="text-xl text-gray-200 animate-on-scroll">Découvrez les services exceptionnels du prestatire
                </p>
            </div>
        </div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div v-else-if="error1" class="text-center mt-20 mb-8 animate-on-scroll">
                <div class="bg-red-50 border border-red-200 rounded-lg p-6 inline-block">
                    <h3 class="text-lg font-medium text-red-800 mb-2">Erreur lors du chargement</h3>
            ...
            <div v-else-if="hasData" class="animate-on-scroll">
                ...
                <!-- Bio et coordonnées -->
                               <div class="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-100 animate-on-scroll">
                        ...
                        <div class="lg:w-1/3">
                            <div class="sticky top-6">
                                <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-6 border border-amber-200 shadow-sm">
                                    <h2 class="text-xl font-semibold text-gold mb-4">Coordonnées</h2>
                                    <div class="space-y-4">
                                        <div class="flex items-start gap-3">
                                            ...
                                            <span class="text-gray-700 mt-1">{{
                                                providerDetails?.enterprise?.phone_number || "Non renseigné"
                                            }}</span>
                                        </div>
                                        <div class="flex items-start gap-3">
                                            <div class="bg-amber-100 p-2 rounded-full">
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <span class="text-gray-700 mt-1">
                                                {{ providerDetails?.served_at_home ? "À domicile" : "En institut" }}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-6">
                                        <button @click="toggleAvailability"
                                            ...
                                            :class="providerDetails?.is_active
                                            ...
                                            <Dot :class="providerDetails?.is_active
                                                ...
                                                " class="w-6 h-6 -ml-1" />
                                            {{
                                                providerDetails?.is_active ? "Disponible maintenant" : "Indisponible"
                                            }}
                                        </button>
                                    </div>
                                </div>
                ...
                <h2 class="text-xl font-semibold text-slate-900 mb-4">Services proposés par l'entreprise</h2>
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex items-center">
                                        <img :alt="service.name"
                                            class="w-10 h-10 rounded-full object-cover mr-3 border-2 border-gold"
                                            :src="service.medias?.link || '/placeholder-image.jpg'" />
                                    ...
                                    <div class="flex justify-between items-center">
                                        <div class="flex items-center mt-1">
                                            ...
                                            <span class="ml-1 text-sm text-gold">{{ service.rating }}</span>
                ...
                <div v-else class="text-center mt-20 mb-8">Aucun service disponible</div>
            </div>
